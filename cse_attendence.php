...
if (isset($_POST['submit'])) {
    ...
    echo "<script>
    alert('Attendence Submited Successfully⏺️');
    </script>";
}
    ...
    <form action="cse_attendence.php" method="post">
        <table class="table">
            ...
            <tbody>
                ...
                error_reporting(0);
                $conn = mysqli_connect('localhost', 'root', '', 'attendance');
                $result = mysqli_query($conn, "select * from student where dept_id=1");
                $serialnumber = 0;
                $counter = 0;
                while ($row = mysqli_fetch_array($result)) {
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="radio" name="attendence_status[<?php echo $counter; ?>]" id="" value="present"
                                required checked>Present
                            <input type="radio" name="attendence_status[<?php echo $counter; ?>]" id="" value="absent"
                                required>Absent
                        </td>
